...
export default function AdminDashboardPage() {
  ...
  if (!isAdmin) {
    return (
      <div className="max-w-md mx-auto my-20 p-8 rounded-2xl border border-zinc-800 bg-zinc-900/80 text-center space-y-4">
        <ShieldAlert className="w-10 h-10 text-amber-400 mx-auto" />
        <h1 className="text-xl font-bold text-white">Administrator Access Required</h1>
        <p className="text-xs text-zinc-400">
          This control center requires verified administrator credentials. Frontend hiding is not security.
        </p>
        <Link
          href="/profile"
          className="inline-block px-4 py-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-xs font-semibold text-white transition"
        >
          Go to Profile (Switch to Admin in QA Switcher)
        </Link>
      </div>
    );
  }
